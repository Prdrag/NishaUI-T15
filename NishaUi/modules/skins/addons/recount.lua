...
local function SkinFrame(frame)
	...
	frame.Title:SetFont(unpack(T.Fonts.skin.setfont))
	...
	frame.CloseButton.t:SetFont(unpack(T.Fonts.aHotkey.setfont))
...
Recount.UpdateBarTextures = function(self)
	for k, v in pairs(Recount.MainWindow.Rows) do
		...
		v.LeftText:SetFont(unpack(T.Fonts.skin.setfont))
		v.RightText:SetPoint("RIGHT", -4, 1)
		v.RightText:SetFont(unpack(T.Fonts.skin.setfont))
	end
end
...
for i = 1, getn(MWbuttons) do
	...
	if button then
		...
		button:FontString("text", unpack(T.Fonts.aHotkey.setfont))
