	...
	GameTooltip:HookScript("OnTooltipSetUnit", function(self)
		...
		if unit then
			if UnitIsPlayer(unit) and not UnitIsEnemy(unit, "player") then
				...
				local spellID
				if T.myclass == "DRUID" and T.level >= 87 and uclass ~= "DRUID" then
					spellID = symbiosis.grant[uclass][spec]
				elseif T.myclass ~= "DRUID" and (uclass == "DRUID" and ulevel >= 87) then
					spellID = symbiosis.gain[T.myclass][spec]
				end
