...
local function CreateAuraIcon(parent)
	...
	button.count:SetFont(unpack(T.Fonts.aHotkey.setfont))
...
local function UpdateAuraIcon(button, unit, index, filter)
	...
	button.cd:SetScript("OnUpdate", function(self) if not button.cd.timer then self:SetScript("OnUpdate", nil) return end button.cd.timer.text:SetFont(unpack(T.Fonts.aHotkey.setfont)) button.cd.timer.text:SetShadowColor(0, 0, 0, 0.4) end)
	button:Show()
end
...
local function UpdateObjects(frame)
	...
	if C["nameplate"].debuffs == true then
		...
		frame.icons:SetBackdropColor(0,0,0,0)
...
local function SkinObjects(frame)
	...
	hp.level:SetFont(unpack(T.Fonts.aHotkey.setfont))
	...
	if C["nameplate"].showhealth == true then
		...
		hp.value:SetFont(unpack(T.Fonts.aHotkey.setfont))
	...
	hp.name:SetFont(unpack(T.Fonts.aHotkey.setfont))
	...
	cb.time:SetFont(unpack(T.Fonts.aHotkey.setfont))
	...
	cb.name:SetFont(unpack(T.Fonts.aHotkey.setfont))
