	...
	elseif( UnitHasVehicleUI( "player" ) ) then
		...
		if( C["actionbar"].vertical_rightbars == true ) then
			...
			if( TukuiSaved.rightbars >= 1 ) then
				TukuiBags:SetPoint( "BOTTOMRIGHT", TukuiRightBar, "TOPRIGHT", 0, 3 )
			else
				TukuiBags:SetPoint( "BOTTOMRIGHT", G.Panels.RightChatBackground, "TOPRIGHT", 0, 3 )
			end
		end
...
local function BagsLayout( self, lb )
	...
	if( lb ) then
		...
		f.editbox:SetFont(unpack(T.Fonts.movers.setfont))
		f.detail:SetFont(unpack(T.Fonts.movers.setfont))
		f.gold:SetFont(unpack(T.Fonts.movers.setfont))
