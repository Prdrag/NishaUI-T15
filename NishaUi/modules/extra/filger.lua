...
C["positions"] = {
	player_buff_icon = { "LEFT", FilgerPlayerBuffs, "LEFT", 0, 0 },
	player_proc_icon = { "LEFT", FilgerPlayerProccs, "LEFT", 0, 0 },
	special_proc_icon = { "RIGHT", Filgerspecialprocicon, "RIGHT", 0, 0 },
	target_debuff_icon = { "LEFT", FilgerTargetDebuffs, "LEFT", 0, 0 },
...
C["spells"] = {
	["DEATHKNIGHT"] = {
		{
			...
			Position = {unpack(C["positions"].player_buff_icon)},
			Enable = C.filger.buffs,
		...
		},
		{
			...
			Alpha = 1,
			IconSize = C["filger"].iconsize,
			Position = {unpack(C["positions"].player_proc_icon)},
			Enable = C.filger.procs,
		...
		},
		{
			...
			Alpha = 1,
			IconSize = C["filger"].iconsize,
			Position = {unpack(C["positions"].cooldown)},
			Enable = C.filger.cd,
	...
	["DRUID"] = {
		...
		},
		{
			...
			Alpha = 1,
			IconSize = C["filger"].iconsize,
			Position = {unpack(C["positions"].target_debuff_icon)},
			Enable = true,
	...
	["ALL"] = {
		{
			...
			Direction = C.filger.specialbuffdirection,
			Mode = "ICON",
			Interval = 3,
			Alpha = 1,
			IconSize = C["filger"].iconsize,
			Position = {unpack(C["positions"].special_proc_icon)},
			Enable = C.filger.specialbuff,
		...
		},
		{
			...
			Alpha = 1,
			IconSize = 60,
			Position = {unpack(C["positions"].pve_debuff)},
			Enable = C.filger.pvepvpdebuffs,
		...
		},
		{
			...
			Alpha = 1,
			IconSize = C["filger"].iconsize,
			Position = {unpack(C["positions"].target_buff_icon)},
			Enable = C.filger.targetbuffs,
...
--		C["positions"].player_buff_icon = {"BOTTOMRIGHT", "oUF_Player", "TOPRIGHT", 2, 173}
...
local add = function(place, spell)
	for class, _ in pairs(C["spells"]) do
		if class == T.myclass then
			for i = 1, #C["spells"][class], 1 do
				if C["spells"][class][i]["Name"] == place then
					table.insert(C["spells"][class][i], spell)
				end
			end
		end
	end
...
function Filger:DisplayActives()
		...
		if not bar then
			...
			if self.Mode == "ICON" then
				...
				if bar.count then
					...
					bar.count:SetFont(unpack(T.Fonts.fFont.setfont))
				...
				if bar.time then
					...
					bar.time:SetFont(unpack(T.Fonts.fFont.setfont))
				...
				if bar.spellname then
					...
					bar.spellname:SetFont(unpack(T.Fonts.fFont.setfont))
	...
	for n in pairs(self.sortedIndex) do
		...
		bar:SetWidth(self.IconSize)
		bar:SetHeight(self.IconSize)
...
function Filger:OnEvent(event, unit)
	if event == "SPELL_UPDATE_COOLDOWN" or event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_FOCUS_CHANGED" or event == "PLAYER_ENTERING_WORLD" or event == "UNIT_AURA" and (unit == "target" or unit == "player" or unit == "pet" or unit == "focus") then
		...
		for i = 1, #C["spells"][T.myclass][id], 1 do
			local data = C["spells"][T.myclass][id][i]
			local found = false
...
if C["spells"] and C["spells"]["ALL"] then
	if not C["spells"][T.myclass] then
		C["spells"][T.myclass] = {}
	end

	for i = 1, #C["spells"]["ALL"], 1 do
		local merge = false
		local spellListAll = C["spells"]["ALL"][i]
		local spellListClass = nil
		for j = 1, #C["spells"][T.myclass], 1 do
			spellListClass = C["spells"][T.myclass][j]
		...
		if not merge or not spellListClass then
			table.insert(C["spells"][T.myclass], C["spells"]["ALL"][i])
		else
...
if C["spells"] and C["spells"][T.myclass] then
	for index in pairs(C["spells"]) do
	...
	local idx = {}
	for i = 1, #C["spells"][T.myclass], 1 do
		local jdx = {}
		local data = C["spells"][T.myclass][i]
		...
		for j = 1, #data, 1 do
			...
			if not spn and not data[j].slotID then
				print("|cffff0000WARNING: spell/slot ID ["..(data[j].spellID or data[j].slotID or "UNKNOWN").."] no longer exists! Report this.|r")
				table.insert(jdx, j)
			end
		end
		...
		if #data == 0 then
			print("|cffff0000WARNING: section ["..data.Name.."] is empty! Report this.|r")
			table.insert(idx, i)
		end
	end

	for _, v in ipairs(idx) do
		table.remove(C["spells"][T.myclass], v)
	end
	...
	for i = 1, #C["spells"][T.myclass], 1 do
		...
		if C["filger"].filgerconfig then
			frame.actives = {}
			for j = 1, math.min(C["filger"].maxtesticon, #C["spells"][T.myclass][i]), 1 do
				local data = C["spells"][T.myclass][i][j]
				local name, icon
				if data.spellID then
					name, _, icon = GetSpellInfo(data.spellID)
				elseif data.slotID then
			...
			for j = 1, #C["spells"][T.myclass][i], 1 do
				local data = C["spells"][T.myclass][i][j]
